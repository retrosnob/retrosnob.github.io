...
image:PythonBook_ThonnyUI.png[Thonny interface]

**Examples of using the interactive shell.**

image:Thonny_shell.png[Thonny interactive shell]
...
[NOTE]
This is a note. I wonder how long it can be and specifically whether it looks good if there are multiple lines.
...
You are familiar with the idea of operator precedence from mathematics. For instance stem:[5 - 3 :times 2 = -1], whereas `(5 - 3) x 2 = 4`.
...
6. You have to put the f at the end of the float field (third field in the `layout` variable) if you want it to treat the precision as *decimal places*. If you leave out the f then it will be *significant figures* instead.
